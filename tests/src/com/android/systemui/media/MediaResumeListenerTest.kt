...
class MediaResumeListenerTest : SysuiTestCase() {
    ...
    @Mock
    private lateinit var resumeBrowserFactory: ResumeMediaBrowserFactory
    @Mock
    private lateinit var resumeBrowser: ResumeMediaBrowser
    @Mock
    ...
    @Captor
    lateinit var callbackCaptor: ArgumentCaptor<ResumeMediaBrowser.Callback>
    @Captor
    lateinit var actionCaptor: ArgumentCaptor<Runnable>
    @Captor
    lateinit var componentCaptor: ArgumentCaptor<String>
    ...
    private var originalQsSetting = Settings.Global.getInt(
        context.contentResolver,
        Settings.Global.SHOW_MEDIA_ON_QUICK_SETTINGS, 1
    )
    private var originalResumeSetting = Settings.Secure.getInt(
        context.contentResolver,
    ...
    fun setup() {
        ...
        resumeListener = MediaResumeListener(
            mockContext, broadcastDispatcher, executor,
    ...
    fun testWhenNoResumption_doesNothing() {
        ...
        val listener = MediaResumeListener(
            context, broadcastDispatcher, executor, tunerService,
            resumeBrowserFactory, dumpManager, clock
        )
        listener.setManager(mediaDataManager)
        verify(broadcastDispatcher, never()).registerReceiver(
            eq(listener.userChangeReceiver),
    ...
    fun testOnUserUnlock_loadsTracks() {
        ...
        verify(mediaDataManager, times(3)).addResumptionControls(
            anyInt(),
    ...
    fun testLoadComponents_recentlyPlayed_adds() {
        ...
        val resumeListener = MediaResumeListener(
            mockContext, broadcastDispatcher, executor,
            tunerService, resumeBrowserFactory, dumpManager, clock
        )
        resumeListener.setManager(mediaDataManager)
        ...
        verify(mediaDataManager, times(1)).addResumptionControls(
            anyInt(),
            any(), any(), any(), any(), any(), eq(PACKAGE_NAME)
        )
    }
    ...
    fun testLoadComponents_old_ignores() {
        ...
        verify(mediaDataManager, times(0)).addResumptionControls(
            anyInt(),
