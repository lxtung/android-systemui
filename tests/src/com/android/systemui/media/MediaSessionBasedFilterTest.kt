...
public class MediaSessionBasedFilterTest : SysuiTestCase() {
    ...
    @Mock
    private lateinit var mediaListener: MediaDataManager.Listener
    ...
    @Mock
    private lateinit var controller1: MediaController
    @Mock
    private lateinit var controller2: MediaController
    @Mock
    private lateinit var controller3: MediaController
    @Mock
    private lateinit var controller4: MediaController
    ...
    @Mock
    private lateinit var remotePlaybackInfo: PlaybackInfo
    @Mock
    private lateinit var localPlaybackInfo: PlaybackInfo
    ...
    fun noMediaSession_loadedEventNotFiltered() {
        ...
        verify(mediaListener).onMediaDataLoaded(
            eq(KEY), eq(null), eq(mediaData1), eq(true),
            eq(0), eq(false)
        )
    }
    ...
    fun remoteAndLocalHaveDifferentKeys_localLoadedEventFiltered() {
        ...
        // THEN the event is not filtered
        verify(mediaListener).onMediaDataLoaded(
            eq(key1), eq(null), eq(mediaData1), eq(true),
            eq(0), eq(false)
        )
        // WHEN a loaded event is received that matches the local session
        ...
        verify(mediaListener, never())
            .onMediaDataLoaded(
                eq(key2), eq(null), eq(mediaData2), anyBoolean(),
                anyInt(), anyBoolean()
            )
    ...
    fun remoteAndLocalHaveDifferentKeys_remoteSessionWithoutNotification() {
        ...
        // THEN the event is not filtered
        verify(mediaListener).onMediaDataLoaded(
            eq(key2), eq(null), eq(mediaData2), eq(true),
            eq(0), eq(false)
        )
    }
    ...
    fun multipleRemoteSessions_loadedEventNotFiltered() {
        ...
        // THEN the event is not filtered
        verify(mediaListener).onMediaDataLoaded(
            eq(KEY), eq(null), eq(mediaData2), eq(true),
            eq(0), eq(false)
        )
    }
    ...
    fun doNotFilterDuringKeyMigration() {
        ...
        verify(mediaListener).onMediaDataLoaded(
            eq(key2), eq(key1), eq(mediaData2), eq(true),
            eq(0), eq(false)
        )
    }
    ...
    fun filterAfterKeyMigration() {
        ...
        // THEN the key migration event is fired
        verify(mediaListener).onMediaDataLoaded(
            eq(key2), eq(null), eq(mediaData1), eq(true),
            eq(0), eq(false)
        )
    }
}
