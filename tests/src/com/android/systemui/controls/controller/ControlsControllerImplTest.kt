...
class ControlsControllerImplTest : SysuiTestCase() {
    ...
    companion object {
        ...
        private val TEST_STRUCTURE_INFO = StructureInfo(
            TEST_COMPONENT,
        ...
        private val TEST_STRUCTURE_INFO_2 = StructureInfo(
            TEST_COMPONENT_2,
    ...
    fun testOnActionResponse() {
        ...
        verify(uiController).onActionResponse(
            TEST_COMPONENT, TEST_CONTROL_ID,
    ...
    fun testLoadForComponent_noFavorites() {
        ...
        verify(bindingController).bindAndLoad(
            eq(TEST_COMPONENT),
    ...
    fun testSeedFavoritesForComponentsWithLimit() {
        ...
        for (i in 1..10) {
            controls1.add(
                statelessBuilderFromInfo(
                    ControlInfo(
                        "id1:$i", TEST_CONTROL_TITLE,
                        TEST_CONTROL_SUBTITLE, TEST_DEVICE_TYPE
                    ), "testStructure"
                ).build()
            )
        }
        ...
        for (i in 1..3) {
            controls2.add(
                statelessBuilderFromInfo(
                    ControlInfo(
                        "id2:$i", TEST_CONTROL_TITLE,
                        TEST_CONTROL_SUBTITLE, TEST_DEVICE_TYPE
                    ), "testStructure2"
                ).build()
            )
        }
        controller.seedFavoritesForComponents(
            listOf(TEST_COMPONENT, TEST_COMPONENT_2),
            Consumer { resp ->
                responses.add(resp)
            })

        verify(bindingController).bindAndLoadSuggested(
            eq(TEST_COMPONENT),
            capture(controlLoadCallbackCaptor)
        )
