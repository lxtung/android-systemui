...
open class DisplayCutoutBaseView : View, RegionInterceptableView {
    ...
    @JvmField
    protected var pendingConfigChange = false
    @JvmField
    protected val paint = Paint()
    @JvmField
    protected val cutoutPath = Path()

    @JvmField
    protected var showProtection = false

    @VisibleForTesting(otherwise = VisibleForTesting.PROTECTED)
    @JvmField
    val protectionRect: RectF = RectF()

    @VisibleForTesting(otherwise = VisibleForTesting.PROTECTED)
    @JvmField
    val protectionPath: Path = Path()
    ...
    override fun getInterceptRegion(): Region? {
        ...
        cutoutBounds.op(
            rootView.left, rootView.top, rootView.right, rootView.bottom,
            Region.Op.INTERSECT
        )
        return cutoutBounds
    }
    ...
    protected open fun drawCutoutProtection(canvas: Canvas) {
        if (cameraProtectionProgress > HIDDEN_CAMERA_PROTECTION_SCALE &&
            ...
            canvas.scale(
                cameraProtectionProgress, cameraProtectionProgress,
    ...
    open fun enableShowProtection(show: Boolean) {
        ...
        cameraProtectionAnimator?.addUpdateListener(ValueAnimator.AnimatorUpdateListener { animation: ValueAnimator ->
    ...
    companion object {
        ...
        @JvmStatic
        protected fun transformPhysicalToLogicalCoordinates(
