        ...
        if (timeSinceActive < smartspaceMaxAgeMillis) {
            ...
            if (shouldReactivate) {
                ...
                logger.logRecommendationActivated(
                    mediaData.appUid, mediaData.packageName,
                    mediaData.instanceId
                )
                listeners.forEach {
                    it.onMediaDataLoaded(
                        lastActiveKey, lastActiveKey, mediaData,
    ...
    fun onSwipeToDismiss() {
        ...
        if (smartspaceMediaData.isActive) {
            ...
            if (dismissIntent == null) {
                Log.w(
                    TAG, "Cannot create dismiss action click action: " +
                            "extras missing dismiss_intent."
                )
            } else if (dismissIntent.component != null &&
                dismissIntent.component.getClassName()
