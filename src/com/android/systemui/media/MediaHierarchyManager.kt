...
class MediaHierarchyManager @Inject constructor(
    ...
    private var animator = ValueAnimator.ofFloat(0.0f, 1.0f).apply {
        ...
        addUpdateListener {
            ...
            if (isCrossFadeAnimatorRunning) {
                animationCrossFadeProgress = MathUtils.lerp(
                    animationStartCrossFadeProgress, 1.0f,
                    animatedFraction
                )
                ...
                currentAlpha = calculateAlphaFromCrossFade(
            ...
            interpolateBounds(
                animationStartBounds, targetBounds, boundsProgress,
    ...
    private fun interpolateBounds(
        ...
        val left = MathUtils.lerp(
            ...
            endBounds.left.toFloat(), progress
        ).toInt()
        val top = MathUtils.lerp(
            startBounds.top.toFloat(),
            endBounds.top.toFloat(), progress
        ).toInt()
        val right = MathUtils.lerp(
            startBounds.right.toFloat(),
            endBounds.right.toFloat(), progress
        ).toInt()
        val bottom = MathUtils.lerp(
            startBounds.bottom.toFloat(),
            endBounds.bottom.toFloat(), progress
        ).toInt()
...
@IntDef(
    prefix = ["TRANSFORMATION_TYPE_"], value = [
