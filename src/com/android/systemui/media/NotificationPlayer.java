...
public class NotificationPlayer implements OnCompletionListener, OnErrorListener {
        ...
        try {
            ...
            synchronized (mCompletionHandlingLock) {
                ...
                if ((mLooper != null)
                    ...
                    if (DEBUG) {
                        Log.d(mTag, "in startSound quitting looper " + mLooper);
                    }
                    mLooper.quit();
                }
    ...
    public void onCompletion(MediaPlayer mp) {
        ...
        synchronized (mCmdQueue) {
            synchronized (mCompletionHandlingLock) {
                if (DEBUG) {
                    Log.d(mTag, "onCompletion queue size=" + mCmdQueue.size());
                }
                if ((mCmdQueue.size() == 0)) {
                    if (mLooper != null) {
                        if (DEBUG) {
                            Log.d(mTag, "in onCompletion quitting looper " + mLooper);
                        }
                        mLooper.quit();
                    }
        ...
        public void run() {
            ...
            synchronized (this) {
                ...
                try {
                    ...
                    if (DEBUG) {
                        Log.d(mTag, "notification will be delayed by "
                    ...
                    player.start();
                    if (DEBUG) {
                        Log.d(mTag, "player.start piid:" + player.getPlayerIId());
                    }
                } catch (Exception e) {
                    if (player != null) {
                        player.release();
