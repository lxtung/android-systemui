...
private fun allowMediaRecommendations(context: Context): Boolean {
    val flag = Settings.Secure.getInt(
...
class MediaDataManager(
    ...
    private val themeText = com.android.settingslib.Utils.getColorAttr(
        context,
    ...
    constructor(
        ...
        context, backgroundExecutor, foregroundExecutor, mediaControllerFactory,
    ...
    fun addResumptionControls(
        ...
        backgroundExecutor.execute {
            loadMediaDataInBgForResumption(
                userId, desc, action, token, appName, appIntent,
                packageName
            )
        }
    }
    ...
    private fun updateState(key: String, state: PlaybackState) {
        mediaEntries.get(key)?.let {
            ...
            val actions = createActionsFromState(
                it.packageName,
        ...
        foregroundExecutor.executeDelayed(
            {
                notifySmartspaceMediaDataRemoved(
                    smartspaceMediaData.targetId, immediately = true
                )
            }, delay
        )
    }
    ...
    fun loadMediaDataInBg(
        ...
        foregroundExecutor.execute {
            ...
            onMediaDataLoaded(
                key, oldKey, MediaData(
                    sbn.normalizedUserId, true, app,
    ...
    private fun createActionsFromNotification(sbn: StatusBarNotification):
        ...
        if (actionsToShowCollapsed.size > MAX_COMPACT_ACTIONS) {
            Log.e(
                TAG, "Too many compact actions for ${sbn.key}," +
        ...
        if (actions != null) {
            for ((index, action) in actions.withIndex()) {
                if (index == MAX_NOTIFICATION_ACTIONS) {
                    Log.w(
                        TAG, "Too many notification actions for ${sbn.key}," +
                ...
                val mediaActionIcon = if (action.getIcon()?.type == Icon.TYPE_RESOURCE) {
                    Icon.createWithResource(sbn.packageName, action.getIcon()!!.resId)
                } else {
                    action.getIcon()
    ...
    private fun createActionsFromState(
        ...
        val prevButton = getStandardAction(
            controller, state.actions,
            PlaybackState.ACTION_SKIP_TO_PREVIOUS
        )
        val nextButton = getStandardAction(
            controller, state.actions,
    ...
    fun onNotificationRemoved(key: String) {
        ...
        if (useMediaResumption && removed?.resumeAction != null && removed.isLocalSession()) {
            ...
            val updated = removed.copy(
                token = null, actions = listOf(resumeAction),
    ...
    private fun packageName(target: SmartspaceTarget): String? {
        ...
        for (recommendation in recommendationList) {
            ...
            extras?.let {
                it.getString(EXTRAS_MEDIA_SOURCE_PACKAGE_NAME)
