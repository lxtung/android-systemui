...
class MediaCarouselController @Inject constructor(
    ...
    init {
        ...
        mediaCarouselScrollHandler = MediaCarouselScrollHandler(
            mediaCarousel, pageIndicator,
                ...
                if (addOrUpdatePlayer(key, oldKey, data, isSsReactivated)) {
                    ...
                    MediaPlayerData.players().forEachIndexed { index, it ->
                        if (it.recommendationViewHolder == null) {
                            it.mSmartspaceId = SmallHash.hash(
                                it.mUid +
    ...
    private fun inflateSettingsButton() {
        val settings = LayoutInflater.from(context).inflate(
            R.layout.media_carousel_settings_button,
            mediaFrame, false
        ) as View
    ...
    private fun inflateMediaCarousel(): ViewGroup {
        val mediaCarousel = LayoutInflater.from(context).inflate(
            R.layout.media_carousel,
            UniqueObjectHostView(context), false
        ) as ViewGroup
    ...
    private fun addOrUpdatePlayer(
        ...
        if (existingPlayer == null) {
            ...
            newPlayer.mediaViewHolder?.player?.layoutParams = lp
    ...
    private fun addSmartspaceMediaRecommendations(
        ...
        newRecs.recommendationViewHolder?.recommendations?.layoutParams = lp
    ...
    private fun updatePageIndicator() {
        val numPages = mediaContent.childCount
    ...
    fun logSmartspaceCardReported(
        ...
        val cardinality = mediaContent.childCount
        surfaces.forEach { surface ->
            ...
            if (DEBUG) {
                Log.d(
                    TAG, "Log Smartspace card event id: $eventId instance id: $instanceId" +
