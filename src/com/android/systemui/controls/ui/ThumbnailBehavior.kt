...
class ThumbnailBehavior : Behavior {
    ...
    override fun bind(cws: ControlWithState, colorOffset: Int) {
        ...
        clipLayer.level = if (enabled) MAX_LEVEL else MIN_LEVEL
        ...
        if (template.isActive) {
            ...
            cvh.bgExecutor.execute {
                val drawable = template.thumbnail.loadDrawable(cvh.context)
                cvh.uiExecutor.execute {
                    val radius = cvh.context.resources
                        .getDimensionPixelSize(R.dimen.control_corner_radius).toFloat()
                    clipLayer.drawable = CornerDrawable(drawable, radius)
                    clipLayer.colorFilter = BlendModeColorFilter(
                        cvh.context.resources
