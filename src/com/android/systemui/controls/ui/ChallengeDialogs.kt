...
object ChallengeDialogs {
    ...
    fun createPinDialog(
        ...
        val (title, instructions) = if (useRetryStrings) {
            ...
            Pair(
                res.getString(R.string.controls_pin_verify, cvh.title.text),
        ...
        return object : AlertDialog(cvh.context, STYLE) {
            ...
            setOnShowListener(DialogInterface.OnShowListener { _ ->
                ...
                useAlphaCheckBox.isChecked = useAlphaNumeric
    ...
    fun createConfirmationDialog(cvh: ControlViewHolder, onCancel: () -> Unit): Dialog? {
        ...
        val builder = AlertDialog.Builder(cvh.context, STYLE).apply {
            ...
            setTitle(
                res.getString(
                    R.string.controls_confirmation_message, cvh.title.text
                )
            )
            setPositiveButton(
                android.R.string.ok,
    ...
    private fun addChallengeValue(action: ControlAction, challengeValue: String): ControlAction {
        val id = action.templateId
        return when (action) {
            is BooleanAction -> BooleanAction(id, action.newState, challengeValue)
            is FloatAction -> FloatAction(id, action.newValue, challengeValue)
            is CommandAction -> CommandAction(id, challengeValue)
            is ModeAction -> ModeAction(id, action.newMode, challengeValue)
