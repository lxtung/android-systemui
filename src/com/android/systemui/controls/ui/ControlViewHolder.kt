...
class ControlViewHolder(
    ...
    init {
        val ld = layout.background as LayerDrawable
        ld.mutate()
    ...
    fun bindData(cws: ControlWithState, isLocked: Boolean) {
        ...
        if (controlStatus == Control.STATUS_UNKNOWN || controlStatus == Control.STATUS_NOT_FOUND) {
            title.text = cws.ci.controlTitle
            subtitle.text = cws.ci.controlSubtitle
        } else {
            cws.control?.let {
                title.text = it.title
    ...
    private fun updateContentDescription() {
        layout.contentDescription = "${title.text} ${subtitle.text} ${status.text}"
    }
    ...
    fun usePanel(): Boolean {
        return deviceType in FORCE_PANEL_DEVICES ||
    ...
    private fun animateBackgroundChange(
        ...
        val (newClipColor, newAlpha) = if (enabled) {
            ...
            val color = cws.control?.customColor?.let {
        ...
        stateAnimator = ValueAnimator.ofInt(clipLayer.alpha, newAlpha).apply {
            addUpdateListener {
                ...
                val updatedClipColor = ColorUtils.blendARGB(
                    oldClipColor, newClipColor,
                    it.animatedFraction
                )
                val updatedBaseColor = ColorUtils.blendARGB(
                    oldBaseColor, newBaseColor,
                    it.animatedFraction
                )
