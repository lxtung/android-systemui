...
class ControlsUiControllerImpl @Inject constructor(
    ...
    private val onSeedingComplete = Consumer<Boolean> { accepted ->
        if (accepted) {
    ...
    private fun reload(parent: ViewGroup) {
        ...
        fadeAnim.interpolator = AccelerateInterpolator(1.0f)
        fadeAnim.duration = FADE_IN_MILLIS
        fadeAnim.addListener(object : AnimatorListenerAdapter() {
            override fun onAnimationEnd(animation: Animator) {
                ...
                showAnim.interpolator = DecelerateInterpolator(1.0f)
                showAnim.duration = FADE_IN_MILLIS
                showAnim.start()
            }
        })
        fadeAnim.start()
    ...
    private fun showSeedingView(items: List<SelectionItem>) {
        ...
        subtitle.text = context.resources.getString(R.string.controls_seeding_in_progress)
    }
    ...
    override fun onRefreshState(componentName: ComponentName, controls: List<Control>) {
        ...
        controls.forEach { c ->
            controlsById.get(ControlKey(componentName, c.controlId))?.let {
                Log.d(ControlsUiController.TAG, "onRefreshState() for id: " + c.controlId)
                iconCache.store(componentName, c.controlId, c.customIcon)
                val cws = ControlWithState(componentName, it.ci, c)
                val key = ControlKey(componentName, c.controlId)
...
private data class SelectionItem(
    ...
    fun getTitle() = if (structure.isEmpty()) {
        appName
    } else {
        structure
    }
