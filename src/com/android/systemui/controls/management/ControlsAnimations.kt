        ...
        return object : LifecycleObserver {
            ...
            init {
                ...
                if (translationY == -1f) {
                    translationY = view.context.resources.getDimensionPixelSize(
                        R.dimen.global_actions_controls_y_translation
                    ).toFloat()
                }
            }
            ...
            fun setup() {
                with(window) {
                    ...
                    enterTransition = enterWindowTransition(view.id)
                    exitTransition = exitWindowTransition(view.id)
                    reenterTransition = enterWindowTransition(view.id)
                    returnTransition = exitWindowTransition(view.id)
                }
            }
