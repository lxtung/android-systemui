...
public class GestureRecorder {
    ...
    private final LinkedList<Gesture> mGestures;
    ...
    public void save() {
        synchronized (mGestures) {
            try {
                ...
                if (DEBUG) {
                    Log.v(TAG, String.format("Wrote %d complete gestures to %s", mLastSaveLen,
                            mLogfile));
                }
            } catch (IOException e) {
    ...
    public void dump(PrintWriter pw, String[] args) {
        save();
        if (mLastSaveLen >= 0) {
            pw.println(mLastSaveLen + " gestures written to " + mLogfile);
        } else {
    ...
    public class Gesture {
        ...
        private final LinkedList<Record> mRecords = new LinkedList<Record>();
        private final HashSet<String> mTags = new HashSet<String>();
        ...
        public class TagRecord extends Record {
            ...
            public String toJson() {
                return String.format("{\"type\":\"tag\", \"time\":%d, \"tag\":\"%s\", " +
                                "\"info\":\"%s\"}",
                        this.time,
                        this.tag,
                        this.info
