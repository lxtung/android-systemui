...
public class NotificationLockscreenUserManagerImpl implements
    ...
    //    public void updatePublicMode() {
    //        //TODO: I think there may be a race condition where mKeyguardViewManager.isShowing
    //         () returns
    //        // false when it should be true. Therefore, if we are not on the SHADE, don't even
    //        bother
    //        // asking if the keyguard is showing. We still need to check it though because
    //        showing the
    //        // camera on the keyguard has a state of SHADE but the keyguard is still showing.
    //        final boolean showingKeyguard = mState != StatusBarState.SHADE
    //              || mKeyguardStateController.isShowing();
    ...
    //            boolean isProfilePublic = devicePublic;
    //            if (!devicePublic && userId != getCurrentUserId()) {
    //                // We can't rely on KeyguardManager#isDeviceLocked() for unified profile
    //                challenge
