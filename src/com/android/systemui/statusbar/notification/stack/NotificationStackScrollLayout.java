...
public class NotificationStackScrollLayout extends ViewGroup implements Dumpable {
    ...
    private boolean mHeadsUpAnimatingAway;    private final Runnable mReflingAndAnimateScroll = () -> {
        animateScroll();
    };
    ...
    private boolean shouldHunAppearFromBottom(ExpandableViewState viewState) {
        return !(viewState.yTranslation + viewState.height < mAmbientState.getMaxHeadsUpTranslation());
    }
    ...
    public void dump(PrintWriter pwOriginal, String[] args) {
        ...
        String sb = "[" +
                ...
                " pulsing=" + (mPulsing ? "T" : "f") +
                " expanded=" + (mIsExpanded ? "T" : "f") +
                " headsUpPinned=" + (mInHeadsUpPinnedMode ? "T" : "f") +
                " qsClipping=" + (mShouldUseRoundedRectClipping ? "T" : "f") +
                " qsClipDismiss=" + (mDismissUsingRowTranslationX ? "T" : "f") +
                " visibility=" + DumpUtilsKt.visibilityString(getVisibility()) +
                " alpha=" + getAlpha() +
                " scrollY=" + mAmbientState.getScrollY() +
                " maxTopPadding=" + mMaxTopPadding +
                " showShelfOnly=" + (mShouldShowShelfOnly ? "T" : "f") +
                " qsExpandFraction=" + mQsExpansionFraction +
                " isCurrentUserSetup=" + mIsCurrentUserSetup +
                " hideAmount=" + mAmbientState.getHideAmount() +
                " ambientStateSwipingUp=" + mAmbientState.isSwipingUp() +
                " maxDisplayedNotifications=" + mMaxDisplayedNotifications +
                " intrinsicContentHeight=" + mIntrinsicContentHeight +
                " contentHeight=" + mContentHeight +
                " intrinsicPadding=" + mIntrinsicPadding +
    ...
    private boolean shouldHideParent(View view, @SelectedRows int selection) {
        ...
        if (view instanceof ExpandableNotificationRow) {
            ExpandableNotificationRow row = (ExpandableNotificationRow) view;
            return isVisible(row) && includeChildInClearAll(row, selection);
        }
        return false;
    }
