 ...
 * directly to the keyguard UI is posted to a {@link android.os.Handler} to ensure it is taken on
 * the UI
...
public class KeyguardViewMediator extends CoreStartable implements Dumpable,
     ...
     */
    private boolean mSurfaceBehindRemoteAnimationRunning;    KeyguardUpdateMonitorCallback mUpdateCallback = new KeyguardUpdateMonitorCallback() {
        ...
        public void onDeviceProvisioned() {
            ...
            synchronized (KeyguardViewMediator.this) {
                // If system user is provisioned, we might want to lock now to avoid showing
        // launcher
                        ...
                        if (!mShowing) {
                            if (DEBUG_SIM_STATES) Log.d(TAG,
                                    "INTENT_VALUE_ICC_LOCKED and keygaurd isn't "
                                            + "showing; need to show keyguard so user can enter " +
                            "sim pin");
                            doKeyguardLocked(null);
                        } else {
     ...
     */
    private boolean mSurfaceBehindRemoteAnimationRequested = false;    ViewMediatorCallback mViewMediatorCallback = new ViewMediatorCallback() {
    ...
    private void sendUserPresentBroadcast() {
        synchronized (this) {
            if (mBootCompleted) {
                ...
                mUiBgExecutor.execute(() -> {
                    for (int profileId :
                     um.getProfileIdsWithDisabled(currentUser.getIdentifier())) {
    ...
    public void dump(PrintWriter pw, String[] args) {
        ...
        pw.println(mBootSendUserPresent);
        pw.print("  mExternallyEnabled: ");
        pw.println(mExternallyEnabled);
        pw.print("  mShuttingDown: ");
        pw.println(mShuttingDown);
        pw.print("  mNeedToReshowWhenReenabled: ");
        pw.println(mNeedToReshowWhenReenabled);
        pw.print("  mShowing: ");
        pw.println(mShowing);
        pw.print("  mInputRestricted: ");
        pw.println(mInputRestricted);
        pw.print("  mOccluded: ");
        pw.println(mOccluded);
        pw.print("  mDelayedShowingSequence: ");
        pw.println(mDelayedShowingSequence);
        pw.print("  mExitSecureCallback: ");
        pw.println(mExitSecureCallback);
        pw.print("  mDeviceInteractive: ");
        pw.println(mDeviceInteractive);
        pw.print("  mGoingToSleep: ");
        pw.println(mGoingToSleep);
        pw.print("  mHiding: ");
        pw.println(mHiding);
        pw.print("  mDozing: ");
        pw.println(mDozing);
        pw.print("  mAodShowing: ");
        pw.println(mAodShowing);
        pw.print("  mWaitingUntilKeyguardVisible: ");
        pw.println(mWaitingUntilKeyguardVisible);
        pw.print("  mKeyguardDonePending: ");
        pw.println(mKeyguardDonePending);
        pw.print("  mHideAnimationRun: ");
        pw.println(mHideAnimationRun);
        pw.print("  mPendingReset: ");
        pw.println(mPendingReset);
        pw.print("  mPendingLock: ");
        pw.println(mPendingLock);
        pw.print("  wakeAndUnlocking: ");
        pw.println(mWakeAndUnlocking);
    }
    ...
    private static class StartKeyguardExitAnimParams {
        ...
        @WindowManager.TransitionOldType
        int mTransit;
        long startTime;
