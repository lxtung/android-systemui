...
enum class QSEvent(private val _id: Int) : UiEventLogger.UiEventEnum {
    ...
    @UiEvent(
        doc = "Tile secondary button clicked. " +
    ...
    @UiEvent(
        doc = "Tile visible in Quick Settings panel. The tile may be in a different page. " +
                "It has an instance id and a spec (or packageName)"
    )
    QS_TILE_VISIBLE(392),
    ...
    @UiEvent(
        doc = "Tile visible in Quick Quick Settings panel. " +
                "It has an instance id and a spec (or packageName)"
    )
