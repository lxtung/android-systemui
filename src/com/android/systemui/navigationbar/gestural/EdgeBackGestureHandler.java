...
public class EdgeBackGestureHandler extends CurrentUserTracker
    ...
    private final OverviewProxyService.OverviewProxyListener mQuickSwitchListener =
    ...
    // For debugging
    private final LogArray mPredictionLog = new LogArray(MAX_NUM_LOGGED_PREDICTIONS);
    private final LogArray mGestureLogInsideInsets = new LogArray(MAX_NUM_LOGGED_GESTURES);
    private final LogArray mGestureLogOutsideInsets = new LogArray(MAX_NUM_LOGGED_GESTURES);


    EdgeBackGestureHandler(Context context, OverviewProxyService overviewProxyService,
                           SysUiState sysUiState, PluginManager pluginManager,
    ...
    private boolean isWithinInsets(int x, int y) {
        ...
        // mLogGesture = false.
        return x <= 2 * (mEdgeWidthLeft + mLeftInset)
