...
public class SwipeHelper implements Gefingerpoken {
    ...
    private final float mTouchSlopMultiplier;
    ...
    public boolean onInterceptTouchEvent(final MotionEvent ev) {
        if (mTouchedView instanceof ExpandableNotificationRow) {
            NotificationMenuRowPlugin nmr =
                    ((ExpandableNotificationRow) mTouchedView).getProvider();
            if (nmr != null) {
    ...
    private boolean isAvailableToDragAndDrop(View v) {
        if (mFeatureFlags.isEnabled(Flags.NOTIFICATION_DRAG_TO_CONTENTS)) {
            if (v instanceof ExpandableNotificationRow) {
                ...
                return dragIntent != null && dragIntent.isActivity() && !canBubble;
            }
        }
        return false;
    }
    ...
    public interface Callback {
         ...
         */
        default boolean canChildBeDragged(@NonNull View animView) {
            return true;
        }
    }
}
